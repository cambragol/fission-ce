    ...
    if (hasTwoButtons && dialogType == DIALOG_TYPE_LARGE) {
        if (initializedButtons) {
            ...
            fontDrawText(windowBuf + backgroundFrmImage.getWidth() * (_doneY[dialogType] + 3) + _doneX[dialogType] + 35,
                secondaryButtonText, backgroundFrmImage.getWidth(), backgroundFrmImage.getWidth(), _colorTable[COL_GREENISH_BROWN]);
...
int showLoadFileDialog(char* title, char** fileList, char* dest, int fileListLength, int x, int y, int flags)
{
    ...
    fontDrawText(windowBuffer + LOAD_FILE_DIALOG_DONE_LABEL_Y * backgroundWidth + LOAD_FILE_DIALOG_DONE_LABEL_X, done, backgroundWidth, backgroundWidth, _colorTable[COL_GREENISH_BROWN]);

    // CANCEL
    const char* cancel = getmsg(&messageList, &messageListItem, 103);
    fontDrawText(windowBuffer + LOAD_FILE_DIALOG_CANCEL_LABEL_Y * backgroundWidth + LOAD_FILE_DIALOG_CANCEL_LABEL_X, cancel, backgroundWidth, backgroundWidth, _colorTable[COL_GREENISH_BROWN]);
    ...
    if (title != nullptr) {
        fontDrawText(windowBuffer + backgroundWidth * FILE_DIALOG_TITLE_Y + FILE_DIALOG_TITLE_X, title, backgroundWidth, backgroundWidth, _colorTable[COL_GREENISH_BROWN]);
    }
...
int showSaveFileDialog(char* title, char** fileList, char* dest, int fileListLength, int x, int y, int flags)
{
    ...
    // DONE
    const char* done = getmsg(&messageList, &messageListItem, 100);
    fontDrawText(windowBuffer + backgroundWidth * SAVE_FILE_DIALOG_DONE_LABEL_Y + SAVE_FILE_DIALOG_DONE_LABEL_X, done, backgroundWidth, backgroundWidth, _colorTable[COL_GREENISH_BROWN]);

    // CANCEL
    const char* cancel = getmsg(&messageList, &messageListItem, 103);
    fontDrawText(windowBuffer + backgroundWidth * SAVE_FILE_DIALOG_CANCEL_LABEL_Y + SAVE_FILE_DIALOG_CANCEL_LABEL_X, cancel, backgroundWidth, backgroundWidth, _colorTable[COL_GREENISH_BROWN]);
    ...
    fontDrawText(fileNameBufferPtr, fileNameCopy, backgroundWidth, backgroundWidth, _colorTable[COL_LIME_GREEN]);
        ...
        if (scrollDirection != FILE_DIALOG_SCROLL_DIRECTION_NONE) {
            ...
            while (1) {
                ...
                if (blinkingCounter == 0) {
                    ...
                    int color = blink ? 100 : _colorTable[COL_LIME_GREEN];
                    blink = !blink;
...
static void fileDialogRenderFileList(unsigned char* buffer, char** fileList, int pageOffset, int fileListLength, int selectedIndex, int pitch)
{
    ...
    if (fileListLength != 0) {
        ...
        for (int index = 0; index < fileListLength; index++) {
            int color = index == selectedIndex ? _colorTable[COL_LIGHT_LEMON] : _colorTable[COL_LIME_GREEN];
